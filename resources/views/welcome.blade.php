...
<body class="bg-paper text-ink font-body antialiased overflow-x-hidden" style="-webkit-font-smoothing: antialiased; text-rendering: optimizeLegibility;">
    ...
    <section class="relative pb-[clamp(48px,8vw,96px)]">
        ...
        {{-- Nav --}}
        <div class="max-w-[1160px] mx-auto px-[clamp(20px,5vw,40px)] py-6 flex items-center justify-between gap-4">
            <div class="flex items-center gap-2.5">
                <svg width="22" height="22" viewBox="0 0 22 22" aria-hidden="true"><rect x="2" y="8" width="3.4" height="6" rx="1.7" fill="#0e9d6e"></rect><rect x="9.3" y="3" width="3.4" height="16" rx="1.7" fill="#0e9d6e"></rect><rect x="16.6" y="6" width="3.4" height="10" rx="1.7" fill="#15110d"></rect></svg>
                <span class="font-display font-bold text-lg tracking-tight">Podcast Bounce</span>
            </div>
            <div class="flex items-center gap-6">
                <a href="#" class="text-muted font-semibold text-[15px] hover:text-accent transition-colors no-underline">Blog</a>
                <a href="#join" class="text-ink font-semibold text-[15px] border border-input-border rounded-full px-[18px] py-[9px] hover:border-accent hover:bg-white transition-all no-underline">Join the waitlist</a>
            </div>
        </div>
        ...
        <div class="max-w-[1160px] mx-auto px-[clamp(20px,5vw,40px)] pt-[clamp(24px,5vw,56px)] flex flex-wrap items-center gap-[clamp(40px,6vw,80px)]">
            ...
            <div class="flex-[1_1_460px] min-w-0">
                <span class="inline-flex items-center gap-2 text-[13px] font-semibold tracking-wide text-accent-deep bg-accent-soft rounded-full px-3.5 py-[7px]">
                    <span class="w-[7px] h-[7px] rounded-full bg-accent inline-block"></span>
                    Podcast hosting · private beta soon
                </span>
                ...
                <div class="mt-8 max-w-[480px]">
                    ...
                    @else
                        <form action="{{ route('waitlist.store') }}" method="POST" class="flex gap-2.5 flex-wrap">
                            @csrf
                            <input type="email" name="email" value="{{ old('email') }}" placeholder="you@yourshow.com" aria-label="Email address" required class="flex-[1_1_220px] min-w-0 h-[54px] border border-input-border rounded-[13px] px-4 font-body text-base bg-white text-ink outline-none transition-all focus:border-accent focus:ring-[3px] focus:ring-accent/16">
                            <button type="submit" class="bg-accent text-white border-none rounded-[13px] px-6 h-[54px] font-body font-semibold text-base cursor-pointer whitespace-nowrap transition-all hover:bg-accent-hover hover:-translate-y-px">Join the waitlist</button>
                        </form>
                        @error('email')
                            <p class="mt-2.5 mx-0.5 text-sm text-danger">{{ $message }}</p>
                        @enderror
                        <p class="mt-3 mx-0.5 text-sm text-faint">No spam - just occasional emails between now and launch.</p>
                    @endif
                </div>
            ...
            {{-- Hero visual --}}
            <div class="flex-[1_1_400px] min-w-0">
        ...
        <div class="max-w-[1160px] mx-auto px-[clamp(20px,5vw,40px)]">
            ...
            <div class="mt-[clamp(36px,5vw,56px)] grid grid-cols-[repeat(auto-fit,minmax(300px,1fr))] gap-[18px]">
                ...
                @foreach ($features as $feature)
                    <div class="bg-white border border-hairline rounded-[18px] p-7">
                        <div class="w-[54px] h-[54px] rounded-[14px] bg-accent-soft flex items-center justify-center">
                            <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="#0a6b4a" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">{!! $feature['icon'] !!}</svg>
                        </div>
                        <h3 class="font-display font-semibold text-xl mt-5 mb-2 tracking-tight">{{ $feature['title'] }}</h3>
                        <p class="text-base leading-normal text-muted">{{ $feature['desc'] }}</p>
                    </div>
                @endforeach
    ...
    <section id="join" class="py-[clamp(40px,6vw,72px)]">
        <div class="max-w-[1160px] mx-auto px-[clamp(20px,5vw,40px)]">
            <div class="bg-accent-soft rounded-[28px] px-[clamp(24px,5vw,56px)] py-[clamp(40px,7vw,80px)] text-center">
                <h2 class="font-display font-bold text-[clamp(30px,4.6vw,50px)] leading-[1.05] tracking-[-0.025em]" style="text-wrap: balance;">Be first through the door.</h2>
                <p class="text-[clamp(16px,2vw,19px)] leading-[1.55] text-on-soft mt-[18px] mx-auto max-w-[30em]">Join the waitlist and you'll be the first to know when we launch.<br /> No spam, ever.</p>

                <div class="mt-[30px] mx-auto max-w-[480px]">
                    @if (session('waitlist_success'))
                        <div class="inline-flex items-center gap-3 bg-white border border-accent-border rounded-[14px] px-[22px] py-4">
                            <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#0a6b4a" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M5 13l4 4L19 7"></path></svg>
                            <span class="text-base font-medium text-accent-deep">You're on the list. We'll send you occasional emails between now and launch.</span>
                        </div>
                    @else
                        <form action="{{ route('waitlist.store') }}" method="POST" class="flex gap-2.5 flex-wrap justify-center">
                            @csrf
                            <input type="email" name="email" value="{{ old('email') }}" placeholder="you@yourshow.com" aria-label="Email address" required class="flex-[1_1_220px] min-w-0 h-[54px] border border-accent-border-2 rounded-[13px] px-4 font-body text-base bg-white text-ink outline-none transition-all focus:border-accent focus:ring-[3px] focus:ring-accent/18">
                            <button type="submit" class="bg-accent text-white border-none rounded-[13px] px-6 h-[54px] font-body font-semibold text-base cursor-pointer whitespace-nowrap transition-all hover:bg-accent-hover hover:-translate-y-px">Join the waitlist</button>
                        </form>
                        @error('email')
    ...
    {{-- ============ FOOTER ============ --}}
    <footer class="py-[clamp(40px,5vw,64px)] pb-[clamp(48px,5vw,72px)] border-t border-[#e8e5da] mt-[clamp(24px,4vw,48px)]">
        <div class="max-w-[1160px] mx-auto px-[clamp(20px,5vw,40px)] flex flex-wrap gap-6 items-center justify-between">
            <div class="flex items-center gap-2.5">
